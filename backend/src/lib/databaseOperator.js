...
class dataBaseOperator {
  ...
  static async read(tableName, params) {
    ...
    const whereClause =
      entries.length > 0
        ? `WHERE ${entries.map((elem) => `${elem[0]} = '${elem[1]}'`).join(" AND ")}`
        : ``;
    const res = await dataBaseOperator.db.query(
  ...
  static update(tableName, params, newData) {
    ...
    const updateClause = newDataEntries
      .map((elem) => elem.join(" = "))
      .join(", ");
    dataBaseOperator.db.query(
      `UPDATE ${tableName} SET (${updateClause}) ${whereClause}`,
      (err, res) => {
        if (err) {
  ...
  static create(tableName, params) {
    ...
    console.log(values);
    dataBaseOperator.db.query(
      `INSERT INTO ${tableName} (${keys.join(", ")}) VALUES (${values.map((elem) => `'${elem}'`).join(", ")})`,
      (err, res) => {
        if (err) {
