...
const validateToken = (token, username) => {
    ...
    if(!decodedHeaderObj?.alg || !decodedHeaderObj?.type || decodedHeaderObj.alg !== "HS256" || decodedHeaderObj.type !== "JWT"){
    ...
    if(!decodedPayloadObj?.username || decodedHeaderObj.username != username || decodedHeaderObj.issuedAt > Date.now()){
