...
const user = async (fastify, options, done) => {
  const { databaseProvider } = options;
  ...
  fastify.post("/register", async (request, reply) => {
    ...
    if (await databaseProvider.userExists(data.email, data.username)) {
      reply
        .code(400)
        .send({ message: "This username and(or) email has already been used" });
      return;
    }
    const codedPassword = await hashPassword(data.password);
    await databaseProvider.addUser(data.email, data.username, codedPassword);
    reply
      .code(200)
      .send({ message: "The account has been successfully created!" });
  });
  fastify.post("/login", async (request, reply) => {
    ...
    const credentials = await databaseProvider.getPasswordByEmail(data.email);
    ...
    if (await validatePassword(data.password, credentials[0].user_password)) {
      ...
      reply
        .code(400)
        .send({ message: "Failed to login, maybe wrong credentials!" });
    }
  });
  fastify.get("/:userName", async (request, reply) => {
    ...
    const user = await databaseProvider.userNameExists(userName);
    if (user.length === 0) {
